...
function renderFinanceReport(data) {
  ...
  // 11주차 김은정 500,000원은 액션러닝이 아니라 수료식 사회비이며, 예산 계획에 해당 항목이 없어 별도 표시한다.
      ...
      items: [
        {
          label: "교재·인쇄물",
          filter: raw("준비비"),
          note: "강의 1~3권 교재, 회원수첩 인쇄비(40부, 행일미디어) / 입학식 현수막 제작(행일미디어) / E-브로슈어·웹포스터 제작(레드우드코퍼레이션)",
        },
        {
          label: "수료식",
          filter: raw("인쇄비"),
          note: "수료식 가로현수막·정방형현수막(4개) / 수료증 인쇄(30부) — 계획상 졸업식 항목이나 인쇄비에 합산 처리(1)",
        },
      ],
    },
    {
      ...
      items: [
        {
          label: "다과비",
          filter: raw("다과비"),
          note: "교육기간 커피, 다과비(10회) — 11주차 수료식 다과비 361,100원 포함(1)",
        },
        {
          label: "식비",
          filter: raw("식비"),
          note: "교육 후 식비(석식)(9회, 10주차 졸업여행 중식비 포함) — 11주차 종강파티 석식비 932,000원 포함(1)",
        },
      ],
    },
    {
      ...
      items: [
        {
          label: "기념품",
          filter: raw("기념품"),
          note: "수료생 선물세트 60개 — 현수막·수료증 인쇄비는 인쇄비, 다과비·종강파티 식비는 다과및식비 항목에 합산(1)",
        },
      ],
    },
    {
      group: "교육장 및 회원수첩",
      items: [
        { label: "대관료", filter: raw("대관료"), note: "7개 회차 협회 회의실 예약" },
        {
          label: "원우회 온라인 회원수첩",
          filter: (t) => t.raw_category === "운영비" && t.detail.includes("수첩"),
          note: "1~4기 온라인 회원수첩 호스팅비(슈빅)(2)",
        },
      ],
    },
    {
      group: "주차비",
      items: [{ label: "주차비", filter: raw("주차비"), note: "9회차 관리비 정산" }],
    },
    {
      group: "기타(예비비)",
      items: [
        {
          ...
          note: "9주차 회의실 예약 불가로 C동 지하1층 대회의실 사용(보증금 20만원+회의실 예약비 55만원, 냉방사용료 포함)(3)",
        },
        {
          label: "사회비",
          amount: 500000,
          note: "3기 장학생 황지영 불참으로 3기 원우(김은정)에게 수료식 사회 진행비 추가 지급(4)",
        },
      ],
    },
  ];
  ...
  byId("finance-category-note").innerHTML = [
    "수료식 운영비 중 '현수막 인쇄비'·'식비'·'다과비'는 상단 항목(인쇄비/다과및식비)에 합산 처리",
    "1~4기 온라인 회원수첩 호스팅 비용",
    "9주차 회의실 예약 불가로 C동 지하1층 대회의실 사용(20만원 보증금+55만원 회의실 예약비, 냉방사용료 포함)",
    "3기 장학생 황지영 불참으로 3기 원우 사회 진행비 추가 지급",
  ]
...
function renderBudgetCompare(data) {
  ...
  // 지출 비교 (품의서 "소요예산" 항목 기준, 실제 결산 항목과 1:1 매칭)
  // 2주차 진행비 1,000,000원은 강사료와 합산 세금계산서 처리 -> 액션러닝으로 이동
  // 11주차 사회비 500,000원(김은정)은 예산에 없는 항목 -> 예비비에 대응(각주4 참고)
  const W2_FACILITATION = 1000000;
  const MC_FEE = 500000;
  ...
  const actualByItem = {
    ...
    주차비: sumBy(raw("주차비")),
    예비비:
      sumBy((t) => t.raw_category === "운영비" && t.detail.includes("회의실")) + MC_FEE,
  };
  const expensePlan = {
    강사료: 20750000,
    액션러닝: 7000000,
  ...
  byId("budget-note").innerHTML = [
    "액션러닝은 유일한 대표 6회(1·2·3·4·6·9주차, 각 1,000,000원) + 강사 직접 진행 2회(7주차 김창원·8주차 고영, 각 500,000원) = 7,000,000원. 2주차분은 강사료와 합산 세금계산서 처리되었으나 성격에 맞게 여기로 집계 — 계획 대비 100.0% 정확히 일치",
    "해외전시회 참가는 계획 12명(선정) 대비 실제 참가 8명으로 축소되어 계획보다 낮게 집행됨",
    "1) 수료식(졸업식) 운영비 중 '현수막 인쇄비'·'식비'·'다과비'는 인쇄비/다과및식비 상단 항목에 합산 처리 — 그래서 수료식(졸업식) 실제 항목엔 기념품(2,310,000원)만 남으며, 계획(4,600,000원) 대비 낮게 보이는 것은 절감이 아니라 항목 재배치에 따른 것임",
    "차경 교육장 임차(계획 780천원, 8주 기준)는 7회차 집행 — 7층 공용회의실 5회는 오후 5시간(13:00~18:00) 예약 기준 50,000원, 입학식·수료식은 지하1층 대회의실 5시간 200,000원",
    "2) 원우수첩 호스팅비(계획 132천원)는 1~4기 온라인 회원수첩 호스팅 비용으로 계획과 실제가 정확히 일치(100.0%)",
    "3) 예비비 중 회의실 임차료 750,000원은 9주차 회의실 예약 불가로 C동 지하1층 대회의실 사용(보증금 20만원 + 회의실 예약비 55만원, 냉방사용료 포함)",
    "4) 예비비 중 사회비 500,000원은 3기 장학생 황지영 불참으로 3기 원우(김은정)에게 수료식 사회 진행비를 추가 지급한 금액 — 예산 계획에 없던 항목",
  ]
