...
function renderFinanceReport(data) {
  ...
  // 2주차는 강사료+진행비가 한 건(2,000,000원)으로 세금계산서 처리되어 '강의비,진행비'로 잡혀 있으므로
  // 진행비분 1,000,000원을 액션러닝 쪽으로 옮겨 실제 성격에 맞게 표시한다.
  // 11주차 김은정 500,000원은 액션러닝이 아니라 수료식 사회비이므로 별도 분리한다.
  const W2_FACILITATION = 1000000;
  const MC_FEE = 500000;
  const lectureRaw = sumBy((t) => t.raw_category === "강의비" || t.raw_category === "강의비,진행비");
  const facilitationRaw = sumBy(raw("진행비"));
  ...
  const groups = [
    {
      ...
      items: [
        {
          label: "강사료",
          amount: lectureRaw - W2_FACILITATION,
          note: "10회차 강사료(10주차 졸업여행 제외)",
        },
        {
          label: "액션러닝 진행비(코칭비)",
          amount: facilitationRaw - MC_FEE + W2_FACILITATION,
          note: "총 8회 — 유일한 대표 6회(1·2·3·4·6·9주차, 각 1,000,000원) + 강사 직접 진행 2회(7주차 김창원·8주차 고영, 각 500,000원). 2주차분은 강사료와 합산 세금계산서 처리되었으나 성격에 맞게 여기로 분류",
        },
        {
          label: "수료식 사회비",
          amount: MC_FEE,
          note: "11주차 수료식 사회 진행(김은정)",
        },
      ],
    },
    {
      ...
      items: [
        {
          label: "교재·인쇄물",
          filter: raw("준비비"),
          note: "교재 제본 1,089,000원(1권 264,000·2권 314,600·3권 510,400) + 회원수첩 132,000원(4·8주차 각 30부) + 홍보용 웹포스터·E브로슈어 1,193,500원 + 현수막·배너 200,200원",
        },
      ...
      items: [
        {
          ...
          note: "국내 교육기간 커피·다과 등. 11주차는 수료식 종강파티에 포함",
        },
        {
          label: "식비",
          filter: (t) => t.raw_category === "식비" && t.week_label !== "11주차",
          note: "국내 교육 9회차 석식비(1·2·3·4·6·7·8·9·10주차). 5주차 현지 식비는 해외연수비에, 11주차는 수료식 종강파티에 포함",
        },
      ],
    },
    {
      ...
      items: [
        {
          label: "대관료",
          filter: raw("대관료"),
          note: "7개 회차 — 7층 공용회의실 5회(오후 13:00~18:00, 각 50,000원) + 지하1층 대회의실 2회(입학식·수료식, 각 200,000원)",
        },
        {
          label: "회의실 임차료",
          filter: (t) => t.raw_category === "운영비" && t.detail.includes("회의실"),
          note: "9주차 보증금 및 사용료(기업방문 불발로 현장강의 대체, 계획外 → 예비비 대응)",
        },
      ],
    },
    {
...
function renderBudgetCompare(data) {
  ...
  // 액션러닝(코칭비) 분리: 2주차 진행비 1,000,000원은 강사료와 합산 처리, 11주차 김은정 500,000원은 사회비
  const W2_FACILITATION = 1000000;
  const MC_FEE = 500000;
  const coaching = sumBy(raw("진행비")) - MC_FEE + W2_FACILITATION;

  const actualByItem = {
    강사비:
      sumBy((t) => t.raw_category === "강의비" || t.raw_category === "강의비,진행비") - W2_FACILITATION + MC_FEE,
    코칭비: coaching,
    // 수료식 현수막·수료증은 계획상 졸업식('현수막 등') 항목이므로 인쇄비에서 제외
    인쇄비: sumBy(raw("준비비")),
    "다과 및 식비":
    ...
    사무용품: 0,
    "해외전시회 참가": 19738080,
    // 계획상 졸업식 = 기념품 + 행사용품 + 식비(행사장 임차·식비) + 현수막 등 + 우편료
    졸업식:
      sumBy(raw("기념품")) +
      sumBy((t) => (t.raw_category === "다과비" || t.raw_category === "식비") && t.week_label === "11주차") +
      sumBy(raw("인쇄비")),
    "차경 교육장 임차": sumBy(raw("대관료")),
    "원우수첩 호스팅비": sumBy((t) => t.raw_category === "운영비" && t.detail.includes("수첩")),
    주차비: sumBy(raw("주차비")),
    // 계획에 없던 9주차 회의실 임차료(기업방문 불발로 현장강의 대체)는 예비비로 대응
    예비비: sumBy((t) => t.raw_category === "운영비" && !t.detail.includes("수첩")),
  };
  const expensePlan = {
    강사비: 20750000,
    코칭비: 7000000,
  ...
  byId("budget-note").innerHTML = [
    ...
    "코칭비(계획 7,000천원)는 액션러닝 진행비로, 유일한 대표 6회(1·2·3·4·6·9주차 각 1,000,000원) + 강사 직접 진행 2회(7주차 김창원·8주차 고영 각 500,000원) = 7,000,000원. 2주차분은 강사료와 합산 세금계산서 처리되었으나 성격에 맞게 코칭비로 집계",
    "강사비(실제)에는 11주차 수료식 사회비 500,000원(김은정)이 포함됨 — 액션러닝이 아니므로 코칭비에서 제외",
    "차경 교육장 임차(계획 780천원, 8주 기준)는 실제 7회차 집행 — 7층 공용회의실 5회는 오후 5시간(13:00~18:00) 예약 기준 50,000원, 입학식·수료식은 지하1층 대회의실 5시간 200,000원",
    "원우수첩 호스팅비(계획 132천원)는 원우회 온라인 수첩 사이트 1년 웹호스팅비로 계획과 실제가 정확히 일치",
    "예비비(계획 2,000천원)는 계획에 없던 9주차 회의실 임차료(750,000원, 기업방문 불발로 현장강의 대체)와 행사 운영비(70,000원)에 대응",
    "졸업식(계획 4,600천원)의 세부 구성은 기념품 2,000 + 행사용품 500 + 식비(행사장 임차·식비) 1,500 + 현수막 등 500 + 우편료 100 천원이며, 실제는 기념품 2,310,000 + 11주차 다과·회식비 1,293,100 + 수료식 현수막·수료증 467,500 = 4,070,600원으로 예산 내 집행(88.5%)",
    "※ 11주차(수료식) 당일 총지출 8,582,600원을 졸업식 예산과 직접 비교하면 안 됨 — 그 안에는 강사비(김대식 3,300,000)·수료식 사회비(500,000)·주차비(380,000)·대관료(200,000)가 포함되어 있고, 이들은 계획상 각각 강사비·주차비·교육장 임차 항목에 편성된 비용임",
  ]
